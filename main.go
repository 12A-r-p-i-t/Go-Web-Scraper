...
func GoogleScrape(searchTerm string, languageCode string, countryCode string, proxyString interface{}, pages int, count int, backoff int) ([]SearchResult, error) {
	results := []SearchResult{}
	resultCounter := 0
...
func main() {
	res, err := GoogleScrape("dasdasda", "en", "com", nil, 1, 30, 10)
	if err == nil {
		for _, res := range res {
			fmt.Println(res)
		}
